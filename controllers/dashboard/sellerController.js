...
class sellerController {
  ...
  get_seller_email = async (req, res) => {
    const { sellerId } = req.params;
    ...
    try {
      ...
      // Return the seller's email in the response
      responseReturn(res, 200, { email: seller.email });
    } catch (error) {
  ...
  seller_type_update = async (req, res) => {
    const { sellerId, profileType } = req.body;
    try {
      await sellerModel.findByIdAndUpdate(sellerId, {
        profileType,
      });
      ...
      responseReturn(res, 200, {
        seller,
        message: "seller type update success",
      });
    } catch (error) {
      responseReturn(res, 500, { error: error.message });
    }
